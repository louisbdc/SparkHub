...
function AuthBg({ mousePos }: { mousePos: { x: number; y: number } }) {
  return (
      ...
      <div className="absolute top-[8%] right-[10%] animate-float-delayed opacity-40 dark:opacity-[0.15]">
        <div className="w-64 border-[1.5px] border-zinc-300 dark:border-zinc-700 rounded-2xl p-4 bg-white/50 dark:bg-transparent">
          ...
          <div className="space-y-2">
            {[[65], [40], [80]].map(([w], i) => (
              <div key={i} className="h-2 w-full bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                <div className={`h-full bg-zinc-300 dark:bg-zinc-600 rounded-full`} style={{ width: `${w}%` }} />
              </div>
            ))}
          </div>
...
export default function RegisterPage() {
  ...
  const handleRoleChange = (r: 'dev' | 'client') => {
    setRole(r)
    setValue('role', r)
  }
  ...
  if (register_.data?.emailConfirmationRequired) {
    return (
      <div className="min-h-screen bg-[#fafafa] dark:bg-[#09090b] flex items-center justify-center p-4 overflow-hidden relative text-zinc-900 dark:text-zinc-100">
        <AuthBg mousePos={mousePos} />
        <div className="w-full max-w-[420px] z-10 animate-in fade-in zoom-in-95 duration-700">
          <div className="flex flex-col items-center mb-10">
            <div className="relative">
              <div className="absolute inset-0 bg-zinc-900 blur-2xl opacity-10 dark:opacity-30 scale-150" />
          ...
          <div className="relative">
            <div className="absolute -inset-[1px] bg-gradient-to-b from-white/80 dark:from-white/10 to-transparent rounded-[2.5rem] -z-10" />
            <div className="bg-white/70 dark:bg-zinc-900/80 backdrop-blur-2xl border border-zinc-200/50 dark:border-white/5 rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] p-10 text-center space-y-5">
              <div className="flex justify-center">
                <CheckCircle className="w-10 h-10 text-green-500" />
              </div>
              <div className="space-y-1.5">
                <p className="text-sm font-bold text-zinc-900 dark:text-white">Vérifiez votre email</p>
              ...
              <Link href="/login" className="inline-flex items-center justify-center gap-2 w-full bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 font-bold py-3.5 rounded-2xl shadow-xl hover:opacity-90 transition-all text-sm">
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] dark:bg-[#09090b] flex items-center justify-center p-4 font-sans selection:bg-zinc-200 dark:selection:bg-zinc-800 overflow-hidden relative text-zinc-900 dark:text-zinc-100">
      ...
      <div className="w-full max-w-[420px] z-10 py-10 animate-in fade-in zoom-in-95 duration-700">
        ...
        <div className="relative group">
          ...
          <div className="bg-white/70 dark:bg-zinc-900/80 backdrop-blur-2xl border border-zinc-200/50 dark:border-white/5 rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] dark:shadow-[0_32px_64px_-16px_rgba(0,0,0,0.4)] p-10 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="mb-7">
                <h2 className="text-xl font-bold text-zinc-900 dark:text-white">Bienvenue</h2>
                <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Créez votre espace de travail client.</p>
              </div>

              <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">

                <div className="space-y-1.5 group/field">
                  <label className="text-[11px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 ml-0.5 group-focus-within/field:text-zinc-700 dark:group-focus-within/field:text-zinc-300 transition-colors">Nom complet</label>
                  <input type="text" placeholder="Alice Dupont" autoComplete="name" className={inputCls} {...register('name')} />
                  {errors.name && <p className="text-xs text-red-500 ml-0.5">{errors.name.message}</p>}
                </div>

                <div className="space-y-1.5 group/field">
                  <label className="text-[11px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 ml-0.5 group-focus-within/field:text-zinc-700 dark:group-focus-within/field:text-zinc-300 transition-colors">Email professionnel</label>
                  <input type="email" placeholder="vous@entreprise.com" autoComplete="email" className={inputCls} {...register('email')} />
                  {errors.email && <p className="text-xs text-red-500 ml-0.5">{errors.email.message}</p>}
                </div>

                <div className="space-y-1.5 group/field">
                  <label className="text-[11px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 ml-0.5 group-focus-within/field:text-zinc-700 dark:group-focus-within/field:text-zinc-300 transition-colors">Mot de passe</label>
                  <div className="relative">
                    <input type={showPassword ? 'text' : 'password'} placeholder="••••••••••••" autoComplete="new-password" className={`${inputCls} pr-11`} {...register('password')} />
                    <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 transition-colors">
                ...
                <div className="space-y-1.5 group/field">
                  <label className="text-[11px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 ml-0.5 group-focus-within/field:text-zinc-700 dark:group-focus-within/field:text-zinc-300 transition-colors">Confirmer</label>
                  <div className="relative">
                    <input type={showConfirm ? 'text' : 'password'} placeholder="••••••••••••" autoComplete="new-password" className={`${inputCls} pr-11`} {...register('confirmPassword')} />
                    <button type="button" onClick={() => setShowConfirm((v) => !v)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 transition-colors">
                      {showConfirm ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                    </button>
                  </div>
                  {errors.confirmPassword && <p className="text-xs text-red-500 ml-0.5">{errors.confirmPassword.message}</p>}
                </div>

                {/* Role chips */}
                <div className="space-y-2">
                  <label className="text-[11px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 ml-0.5">Type de compte</label>
                  <div className="grid grid-cols-2 gap-2">
                    {(['dev', 'client'] as const).map((r) => (
                      <button
                        key={r}
                        type="button"
                        ...
                        className={`flex items-center justify-center gap-2 py-2.5 text-xs font-bold rounded-xl border transition-all ${
                          role === r
                            ...
                            : 'bg-transparent border-zinc-200 dark:border-zinc-800 text-zinc-500 hover:bg-zinc-50 dark:hover:bg-zinc-800/50'
                        }`}
                      >
                        {role === r && <CheckCircle2 className="w-3 h-3" />}
                        {r === 'dev' ? 'Développeur' : 'Client'}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                {register_.error && (
                  <div className="rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 px-4 py-3">
                    <p className="text-xs text-red-600 dark:text-red-400">{register_.error.message}</p>
                  </div>
                )}
                ...
                >
                  ...
                  {register_.isPending ? (
                    ...
                    <>
                      <span className="relative text-sm tracking-wide">CRÉER MON ESPACE</span>
        ...
        <div className="mt-8 text-center animate-in fade-in slide-in-from-top-2 duration-1000">
          <div className="group text-sm text-zinc-500 dark:text-zinc-400">
            ...
            <Link href="/login" className="inline-flex items-center ml-1 font-bold text-zinc-900 dark:text-zinc-100 underline-offset-4 hover:underline">
              Se connecter
