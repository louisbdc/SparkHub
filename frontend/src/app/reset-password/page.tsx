...
import { ArrowRight, CheckCircle, Eye, EyeOff, Layout, Loader2, MessageSquare, PenTool, XCircle } from 'lucide-react'
...
export default function ResetPasswordPage() {
  ...
  const onSubmit = (values: FormValues) => {
    ...
    resetPassword.mutate({ accessToken, newPassword: values.password }, { onSuccess: () => setDone(true) })
  }
      ...
      <div className="w-full max-w-[420px] z-10 animate-in fade-in zoom-in-95 duration-700">
              ...
              {tokenError ? (
                <div className="text-center space-y-5 py-2">
                  ...
                  <div className="space-y-1.5">
                    <p className="text-sm font-bold text-zinc-900 dark:text-white">Lien invalide</p>
                    <p className="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed">
                      Ce lien de réinitialisation est invalide ou a expiré.
                    </p>
                  </div>
                  <Link href="/forgot-password" className="inline-flex items-center justify-center gap-2 w-full bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 font-bold py-3.5 rounded-2xl shadow-xl hover:opacity-90 transition-all text-sm">
                ...
                <div className="text-center space-y-5 py-2">
                  ...
                  <div className="space-y-1.5">
                    <p className="text-sm font-bold text-zinc-900 dark:text-white">Mot de passe mis à jour</p>
                    <p className="text-sm text-zinc-500 dark:text-zinc-400">Votre mot de passe a été modifié avec succès.</p>
                  </div>
                ...
                <>
                  <div className="mb-7">
                    <h2 className="text-xl font-bold text-zinc-900 dark:text-white">Nouveau mot de passe</h2>
                    <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Choisissez un mot de passe sécurisé.</p>
                  </div>

                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    <div className="space-y-1.5 group/field">
                      <label className="text-[11px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 ml-0.5 group-focus-within/field:text-zinc-700 dark:group-focus-within/field:text-zinc-300 transition-colors">Nouveau mot de passe</label>
                      <div className="relative">
                        <input type={showPassword ? 'text' : 'password'} placeholder="••••••••••••" autoComplete="new-password" autoFocus className={`${inputCls} pr-11`} {...register('password')} />
                    ...
                    {resetPassword.error && (
                      <div className="rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 px-4 py-3">
                        <p className="text-xs text-red-600 dark:text-red-400">{resetPassword.error.message}</p>
                      </div>
                    )}
                    ...
                    >
                      ...
                      {resetPassword.isPending ? (
                        ...
                        <>
                          <span className="relative text-sm tracking-wide">RÉINITIALISER</span>
        ...
        {!done && !tokenError && (
          <p className="text-center text-sm text-zinc-500 dark:text-zinc-400 mt-7">
            <Link href="/login" className="hover:text-zinc-900 dark:hover:text-white transition-colors font-medium">
