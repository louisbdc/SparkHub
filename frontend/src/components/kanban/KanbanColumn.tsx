...
import { KanbanTicketTree } from './KanbanTicketTree'
...
export function KanbanColumn({
  status, label, tickets, workspaceId, onTicketClick, onTicketEdit, onTicketDelete,
}: KanbanColumnProps) {
  const { setNodeRef, isOver } = useDroppable({ id: status })
  const style = COLUMN_STYLES[status]

  // Only root tickets are draggable; children are rendered nested by KanbanTicketTree
  ...
  return (
    <div className="flex flex-col w-[85vw] sm:w-72 shrink-0 h-full snap-center">
      ...
      <SortableContext items={rootTickets.map((t) => t._id)} strategy={verticalListSortingStrategy}>
        <div
          ref={setNodeRef}
          className={cn(
